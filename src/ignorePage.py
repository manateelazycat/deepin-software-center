...
class Topbar(object):
    ...
    def updateNum(self, upgradeNum):
        ...
        if upgradeNum == 0:
            ...
            markup = ("<span size='%s'>有 </span>" % (LABEL_FONT_SIZE)) + ("<span foreground='%s' size='%s'>%s</span>" % (self.numColor, LABEL_FONT_SIZE, str(upgradeNum))) + ("<span size='%s'> 款软件不再提醒升级</span>" % (LABEL_FONT_SIZE))
